...
float pid_calc(pid_struct_t *pid, float ref, float cur){
  ...
  pid->i_out += pid->ki * pid->err[0] * pid->dt;
  if (pid->dt == 0) {
    pid->d_out = pid->kd * (pid->err[0] - pid->err[1]);
  } else {
    pid->d_out = pid->kd * (pid->err[0] - pid->err[1]) / pid->dt;
  }
  LIMIT_MIN_MAX(pid->i_out, -pid->i_max, pid->i_max);
	if(isnan(pid->i_out))pid->i_out = 0;
  
  pid->out = pid->p_out + pid->i_out + pid->d_out;
  LIMIT_MIN_MAX(pid->out, -pid->out_max, pid->out_max);
  return pid->out;
}
...
float pid_dual_loop(pid_struct_t pid[2], float err, float err_out){
  float pid_in_output;
  pid_in_output = pid_calc(&pid[ANG_LOOP], 0, err);
